...
| `farmingCommands` | dict[str, bool] | Commands set to `true` will be automated. | <details> `{ "potato": true, "steal": true, "trample": false, "cdr": true, "eat": false, "quiz": false, "rankup": true, "prestige": true }` </details> |
...
| `oppositePlatform` | list[str] | Commands/Shop items that should be executed on the opposite platform than `usePotatApi` is set to. Possible values: `potato`, `steal`, `trample`, `cdr`, `eat`, `quiz`, `shop-fertilizer`, `shop-guard`, `shop-cdr`, `shop-quiz` | `[]` |
